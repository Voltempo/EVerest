...
void EvSession::terminate() {
    ...
    log("Control pilot error / plug-out: terminating the V2G session immediately");
    driver_stopped = true;
    signal_session_finished_if_done();
}
...
void EvSession::signal_session_finished_if_done() {
    ...
    log("V2G session finished (driver_stopped=%d, engine_finished=%d)", static_cast<int>(driver_stopped),
