...
TEST_F(EnergyNodeTest, BuildRotatedRootUsageAppliesConfiguredRotation) {
    ...
    const auto rotated = module::energy_grid::build_rotated_root_usage(pm, "312");
...
/// build_rotated_root_usage must leave the reading untouched for the default "123" rotation
TEST_F(EnergyNodeTest, BuildRotatedRootUsageIsNoOpForIdentityRotation) {
    ...
    const auto rotated = module::energy_grid::build_rotated_root_usage(pm, "123");
...
TEST_F(EnergyNodeTest, MergeChildEnergyFlowRequestUpdatesExistingChildInPlace) {
    ...
    auto child_a_it = std::find_if(children.begin(), children.end(),
                                   [](const auto& c) { return c.uuid == "child-a"; });
